...
public class ArrayPartitionI {
     ...
     *      最简单的解决方案是考虑 numsnums 数组的元素每个可能的配对集。
     *  为了生成所有可能的配对，我们使用函数 permute（nums，current_index）。
     *  此函数创建给定数组元素的所有可能排列。
     *
     *  为此，permute将当前元素 current_indexcurrent(i)index的索引作为参数之一，
     *  然后，它将当前元素与数组中的每个其他元素交换，向右移动，以便生成数组元素的新排序。
     *  在完成交换之后，它再次调用 permute，但这次使用数组中下一个元素的索引。返回时，我们反转当前函数调用中的交换。
     *
     * 因此，当到达数组的末尾时，会生成数组元素的新排序。考
     * 虑配对的元素，使得每对的第一个元素来自新数组的前半部分，第二个元素来自数组的后半部分。
     ...
     *      因此，基于上面的讨论，我们遍历哈希表 arrarr。如果当前元素出现 req_i次，并且其中一个元素与右边区域中的其他元素配对（考虑虚拟排序数组），我们考虑当前元素 [freq_i/2]次数以及数组中出现的下一个元素 [freq_i/2]最终总和的次数。
     * 为了传播这个左边对所选数字的影响，我们使用了一个标志 dd。如果当前集合中有剩余元素将被再次考虑，则此标志设置为 1。在从下一组中选择元素时，会考虑已考虑的相同额外元素。
     *
