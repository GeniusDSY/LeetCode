...
public class ReshapeTheMatrix {
     ...
     *      空间复杂度：O(m*n)。使用大小为 m*nm∗n 的结果矩阵。
     */
    ...
    /**
     * 【想法】
     *      这种方法背后的想法如下。你知道二维数组是如何存储在主存中的（本质上是一维）吗？它仅在内部表示为一维阵列。元素nums [i] [j] nums数组通过使用以下形式的索引以一维数组的形式表示：$ nums [n * i + j] ，其中，其中 m 是给定矩阵中的列数。以相反的顺序查看相同的内容，同时将元素放在结果矩阵中的元素中，我们可以使用是给定矩阵中的列数。以相反的顺序查看相同的内容，同时将元素放在结果矩阵中的元素中，我们可以使用 count 变量，该变量对于遍历的每个元素都会递增，就像我们将元素放在一维中一样结果数组。但是，要将变量，该变量对于遍历的每个元素都会递增，就像我们将元素放在一维中一样结果数组。但是，要将 count 转换回列数为转换回列数为 c 的二维矩阵索引，我们可以获得的二维矩阵索引，我们可以获得 res [count / c] [count％c] 的索引，其中的索引，其中 count / c 是行号和是行号和 count％c $是列数字。因此，我们可以节省每一步所需的额外检查。
     * @param nums
     * @param r
     * @param c
     * @return
     * 【复杂度分析】
     *      时间复杂度：O(m*n)。我们只遍历整个矩阵 m*n 。这里，m 和 n 指的是给定矩阵中的行数和列数。
     *      空间复杂度：O(m*n)。使用大小为 m*nm∗n 的矩阵存储结果。
     */
